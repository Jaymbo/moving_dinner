...
export default function InvitationsPanel({
  ...
  async function handleCreateInvitation() {
    try {
      const result = await groups.createInvitation(
        groupId,
        newInvMaxUses ? parseInt(newInvMaxUses) : undefined
      );
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mt-2 mb-2">
        ...
        />
        <button className="btn-sm btn-primary" onClick={handleCreateInvitation}>
          + Neuer Code
        </button>
      </div>
            ...
            <tbody>
              {invitations.map((inv: any) => (
                <tr key={inv.id}>
                  <td>
                    <a
                      href={getJoinLink(inv.code)}
                      target="_blank"
                      rel="noopener noreferrer"
                      style={{ wordBreak: 'break-all' }}
                    >
                      {getJoinLink(inv.code)}
                    </a>
                  </td>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <CopyButton text={getJoinLink(inv.code)} label="📋" />
                  </td>
                </tr>
              ))}
            </tbody>
