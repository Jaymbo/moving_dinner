...
import { recalculateScoresForGroup } from '../services/scoring';
import { recalculateMatrixForGroup } from '../services/matrix';
  ...
  try {
    ...
    const updated = await prisma.group.update({
      ...
      data: {
        ...
        ...(meetingCreation !== undefined && { meetingCreation: meetingCreation === 'all' ? 'all' : 'admin' }),
      },
    });
    res.json(updated);
  } catch (err) {
...
router.post('/:id/leave', requireAuth, requireGroupMember, async (req: AuthRequest, res: Response) => {
  try {
    const id = parseInt(req.params.id, 10);
...
router.get('/:id/members', requireAuth, requireGroupMember, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const members = await prisma.groupMember.findMany({
      where: { groupId },
      include: { user: { select: { id: true, name: true, email: true, isGuest: true, isSuperAdmin: true } } },
      orderBy: { joinedAt: 'asc' },
    });
    res.json(members);
  } catch (err) {
...
router.delete('/:id/members/:userId', requireAuth, requireGroupAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const groupId = parseInt(req.params.id, 10);
...
router.put('/:id/members/:userId/role', requireAuth, requireGroupAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const groupId = parseInt(req.params.id, 10);
    const userId = parseInt(req.params.userId, 10);
    const { role } = req.body;
...
router.post('/:id/invitations', requireAuth, requireGroupAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (expiresAt) {
      ...
      if (isNaN(expiresAtDate.getTime())) { res.status(400).json({ error: 'Invalid expiresAt' }); return; }
    }
...
router.get('/:id/scores', requireAuth, requireGroupMember, async (req: AuthRequest, res: Response) => {
  try {
    const groupId = parseInt(req.params.id, 10);
    const scores = await prisma.score.findMany({
      where: { groupId },
      include: { user: { select: { id: true, name: true, email: true, maxGuests: true, isGuest: true, isSuperAdmin: true } } },
      orderBy: { score: 'asc' },
    });
    res.json(scores);
  } catch (err) {
...
router.get('/:id/matrix', requireAuth, requireGroupMember, async (req: AuthRequest, res: Response) => {
  try {
    const groupId = parseInt(req.params.id, 10);
...
router.post('/:id/recalculate', requireAuth, requireGroupAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const groupId = parseInt(req.params.id, 10);
